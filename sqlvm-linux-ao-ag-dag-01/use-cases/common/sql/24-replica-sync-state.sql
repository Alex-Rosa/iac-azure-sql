...
-- The DMV keeps finished operations too: show only seeding that is still in progress.
WHERE internal_state_desc NOT IN ('Success', 'Failed', 'Canceled')
  AND remote_machine_name LIKE '%' + (SELECT TOP (1) REPLACE(REPLACE(endpoint_url, 'tcp://', ''), ':5022', '')
