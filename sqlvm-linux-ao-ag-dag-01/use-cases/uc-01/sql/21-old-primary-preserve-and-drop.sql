...
--   2. drop the stale distributed AG, 3. take the AG offline, 4. drop the AG, 5. drop its databases,
-- so that node-1 can rejoin as a clean secondary and be re-seeded from the new primary.
-- sqlcmd -v AgName="agsqlvm-node-1" DagName="dagsqlvm-node-1-node-3" BackupDir="/var/opt/mssql/backup" RunId="20260924-150000" SkipBackup="0" -i 21-old-primary-preserve-and-drop.sql
SET NOCOUNT ON;
...
-- The stale distributed AG definition goes first: it references the local AG, and a stale global
-- primary must never push its divergent log to the forwarder. DagName="none" skips this.
IF N'$(DagName)' <> N'none' AND EXISTS (SELECT 1 FROM sys.availability_groups WHERE name = N'$(DagName)' AND is_distributed = 1)
BEGIN
    DROP AVAILABILITY GROUP [$(DagName)];
    SELECT 'DAG_DROPPED=$(DagName)';
END
