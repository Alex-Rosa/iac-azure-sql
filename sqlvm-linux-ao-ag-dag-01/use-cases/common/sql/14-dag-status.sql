...
SET NOCOUNT ON;
SELECT 'DAG_EXISTS=$(DagName)|' + CASE WHEN EXISTS (SELECT 1 FROM sys.availability_groups WHERE name = N'$(DagName)' AND is_distributed = 1) THEN '1' ELSE '0' END;
SELECT 'DAG_MEMBER=$(DagName)|' + ar.replica_server_name + '|' + ar.endpoint_url
...
SELECT 'DAG_DB=$(DagName)|' + ar.replica_server_name + '|' + ISNULL(DB_NAME(drs.database_id), CAST(drs.group_database_id AS varchar(36))) COLLATE DATABASE_DEFAULT
